			...
			$.ajax({
				...
				success: function (data) {
					...
					if (html) {
						...
						aigua.codeMirror.setValue("<svg width=\"640\" height=\"480\" xmlns=\"http://www.w3.org/2000/svg\">\n <!-- Created with SVG-edit - http://svg-edit.googlecode.com/ -->\n</svg>");
					}
      ...
      if(window.editorChange){
		...
		respondToKey: function(cm) {
			...
			if (!aigua.slider.is(':visible') && !$(aigua.miniColorsSelector).is(':visible')) {
				...
				// are we on js mode?
				if (aigua.modes[aigua.currentModeIndex].name == 'html' || aigua.modes[aigua.currentModeIndex].name == 'javascript' ||
					aigua.modes[aigua.currentModeIndex].name == 'json') {
					// are we on a number?
					var number = false;
					var hex = token.string.substring(1, token.string.length - 1);
  				if (token.className == 'string' && !aigua.isHexString(hex)){
            ...
            var re = /[-]?[0-9]+[\.]?[0-9]*/g;
            var m;
            while (m = re.exec(token.string)) {
              var number = m[0];
              start = token.start + token.string.indexOf(number,m.index);
              end = start + number.length;
              if(cursor.ch >= start && cursor.ch <= end){
                // save the original number
                if (aigua.originalNumber == null) {
					    		aigua.originalNumber = Number(number);
    						}
    						break;
              }
            }
						...
						// save the original number
            // if (aigua.originalNumber == null) {
            //  aigua.originalNumber = Number(token.string.substring(1, token.string.length - 1));
            // }

						// select token
						aigua.currentSelectionStart = {line: cursor.line, ch: start};
						aigua.currentSelectionEnd   = {line: cursor.line, ch: end};
...
$(function() {
	...
	// do a slow fade in
	$('#main').fadeIn(1000);
  aigua.onElementChanged = function(content){
    window.editorChange = true;
    aigua.switchMode('html', true);
    aigua.codeMirror.setValue(content);
    window.editorChange = false;
  };
	...
	$('#iframeContainer').append('<iframe src="/editor/svg-editor.html" scrolling="no"></iframe>');
