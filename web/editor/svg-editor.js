  ...
  if (!window.svgEditor) window.svgEditor = function($) {
    ...
    Editor.init = function() {
      ...
      var elementChanged = function(window, elems) {
          ...
          //SVGCHANGE
          if(!window.codeChange){
            parent.aigua.onElementChanged(svgCanvas.getSvgString());
          }
              ...
              if (!btn.includeWith && !btn.list) {
                ...
                if (btn.type == 'mode_flyout') {
                  ...
                  if (!ref_btn.parent().hasClass('tools_flyout')) {
                    ...
                    var show_btn = ref_btn.clone().attr('id', tls_id + '_show').append($('<div>', {
      ...
      var updateToolbar = function() {
          if (selectedElement != null) {
            ...
            default:
              ...
              if ($('#linejoin_' + attr).length != 0) setStrokeOpt($('#linejoin_' + attr)[0]);

              attr = selectedElement.getAttribute("stroke-linecap") || 'butt';

              if ($('#linecap_' + attr).length != 0) setStrokeOpt($('#linecap_' + attr)[0]);
            }
...
/*
      ...
      var linkControlPoints = function() {
          ...
          if (linked) $('#tool_node_link').addClass('push_button_pressed').removeClass('tool_button');
          else $('#tool_node_link').removeClass('push_button_pressed').addClass('tool_button');
      ...
      var clickWireframe = function() {
          ...
          if (wf) $('#tool_wireframe').addClass('push_button_pressed').removeClass('tool_button');
          else $('#tool_wireframe').removeClass('push_button_pressed').addClass('tool_button');
      ...
      window.livecoding = (function() {
        return {
          ...
          renderCode: function(code) {
            window.codeChange = true;
            if(code.match(/<svg[\s\S]*<\/svg>/)){
              $('#svg_source_textarea').val(code);
              editingsource = true;
              saveSourceEditor();
            }
            window.codeChange = false;
          }
        }
      }());
      ...
      var setIconSize = Editor.setIconSize = function(size, force) {
          ...
          if (size != 'm') {
            ...
            //this.style[ua_prefix + 'Transform'] = 'scale(' + scale + ')';
            var prefix = '-' + ua_prefix.toLowerCase() + '-';
            style_str += (sel_toscale + '{' + prefix + 'transform: scale(' + scale + ');}' + ' #svg_editor div.toolset .toolset {' + prefix + 'transform: scale(1); margin: 1px !important;}' // Hack for markers
      ...
      var PaintBox = function(container, type) {
          ...
          var svgdocbox = new DOMParser().parseFromString('<svg xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%"\
      ...
      // Select given tool
      Editor.ready(function() {
        var tool, itool = curConfig.initTool,
